...
std::string 
gp_get_stack_trace( const std::string & file, int line ){
...
void
gp_ptr_call_stack_trace(FILE* out_fp) {
	if(out_fp == gp_null){
		out_fp = stderr;
	}

	void* trace[GP_MAX_CALL_STACK_SZ];
	memset((uint8_t*)trace, 0, GP_MAX_CALL_STACK_SZ * sizeof(void*));

	size_t trace_sz = backtrace(trace, GP_MAX_CALL_STACK_SZ);

	fprintf(out_fp, "trace_size=" GP_PRT_SIZE_T " \n", trace_sz);

	char **stack_strings = backtrace_symbols(trace, trace_sz);
	fprintf(out_fp, "STACK_STRACE------------------------------\n");
	for( size_t ii = 1; ii < trace_sz; ii++ ) {
		if(ii >= GP_MAX_CALL_STACK_SZ){ break; }
		fprintf(out_fp, "%s \n", stack_strings[ii]);
	}
...
bool 
gp_call_assert(char* out_fnam, bool is_assert, bool prt_stck, bool cond, 
		const char* fnam, int line, const char* ck_str, const char* fmt, ...)
{
	...
	if(do_prt){
		...
		if(is_assert && (ck_str != gp_null)){
			fprintf(out_file, "ASSERT '%s' FAILED.\n", ck_str);
		} 
		if(prt_stck){
			gp_ptr_call_stack_trace(out_file);
		}

		if(fmt != NULL){
			char prt_buff[GP_MAX_STR_SZ];
			va_list ap;

			va_start(ap, fmt);
			int size = vsnprintf(prt_buff, GP_MAX_STR_SZ, fmt, ap);
			va_end(ap);

			prt_buff[GP_MAX_STR_SZ - 1] = '\0';
	...
	if((out_fnam != gp_null) && (out_file != stderr)){
		fclose(out_file);
	}
